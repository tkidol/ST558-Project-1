...
players <- players %>% select(currentTeam.id, id, fullName, birthCountry)
...
# Roster for last 2 seasons, high points team (presidents cup) low points eam
...
head((PL_SK_Season), n = 100)

```

# 2020 Player Origin Country vs Penalty Minutes by Conference Analysis

```{r, player origin, cache = TRUE}
# ttt
TeamCodes <- TeamStatsAPI()
TeamIDs <- as.integer(TeamCodes$id)


Players <- lapply(TeamIDs, TeamPlayer)

# All active 2020 players including Team ID & Continent of Origin
PlayerCont <- bind_rows(Players)
PlayerCont$currentTeam.id <- as.integer(PlayerCont$currentTeam.id)
PlayerCont <- mutate(PlayerCont, continent = ifelse((birthCountry == "CAN") | (birthCountry == "USA") | (nchar(birthCountry) == 2), "N.America", "Europe"))
PlayerCont <- mutate(PlayerCont, conference = ifelse((currentTeam.id < 16) | (currentTeam.id == 17) | (currentTeam.id == 29), "Eastern", "Western")) 
PlayerCont <- mutate(PlayerCont, playoffs = ifelse((currentTeam.id %in% c(2, 4, 5, 6, 10, 12, 14, 15, 18:23, 25, 54)), TRUE, FALSE))
PlayerCont <- PlayerCont %>% select(continent, conference, playoffs, everything()) 
head((PlayerCont), n = 100)
```
