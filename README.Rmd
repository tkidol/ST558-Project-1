...
```
```{r, user input}
selectInput("n_ID",label = "Choose a Franchise Id", choices = c(1:38), selected = 26)
```
```{r, numeric input}
id <- FranchiseAPI()[ , "Team ID"]
selectInput("n",label = "Choose a Team ID", choices = id, selected = 26)
